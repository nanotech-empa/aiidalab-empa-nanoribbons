...
class BandsViewerWidget(ipw.VBox):
    ...
    def __init__(self, **kwargs):
        ...
        self.efm_fit_slider = ipw.IntSlider(
            description="Eff. mass fit",
            min=3,
            max=15,
            step=2,
    ...
    def plot_bands(self, ispin):
        ...
        ax_y = bq.Axis(
            label="eV", scale=y_sc, orientation="vertical", grid_lines="solid"
        )
    ...
    def mk_igor_link(self, ispin):
        ...
        filename = (
            self.structure.get_ase().get_chemical_formula()
            + f"_bands_spin{ispin}_pk{self.structure.id}.itx"
        )
    ...
    def igor_bands(self, ispin):
        ...
        with testio as fobj:
            ...
            for i in range(nkpoints):
                ...
                fobj.write(
                    ("\t{:.7f}" * nbands).format(*tosave[:, i])
                )  # other columns the bands
                fobj.write("\r")
            fobj.write("END\r")
            fobj.write(
                f'X SetScale/P x {0},{k_axis[1] - k_axis[0]},"", x1; SetScale y 0,0,"", x1\r'
            )
            for idk in range(nbands):
                fobj.write(
                    ('X SetScale/P x 0,1,"", y{0}; SetScale y 0,0,"", y{0}\r').format(
                        str(idk)
                    )
                )
    ...
    def calc_effective_mass(self, ispin):
        ...
        band = (
            self.bands_array[ispin].transpose()[self.selected_band - 1]
            - self.vacuum_level
        )
        k_axis = np.linspace(0.0, np.pi / self.structure.cell_lengths[0], nkpoints)
        ...
        k_vals_ext = np.concatenate(
            [-np.flip(k_axis, 0)[:-1], k_axis, k_axis[-1] + k_axis[1:]]
        )
...
class CubeArrayData2dViewerWidget(ipw.VBox):
    ...
    def __init__(
        ...
        self.height_slider = ipw.SelectionSlider(
            description="Height",
            options={"---": 0},
            continuous_update=False,
            layout=layout,
        ...
        self.opacity_slider = ipw.FloatSlider(
            description="Opacity",
            value=0.5,
            max=1.0,
            continuous_update=False,
        ...
        self.colormap_slider = ipw.FloatSlider(
            value=1.0,
            min=0.05,
            max=5.0,
            step=0.05,
        ...
        self.axis = ipw.ToggleButtons(
            options=[("X", [1, 2, 0]), ("Y", [2, 0, 1]), ("Z", [0, 1, 2])],
            value=[0, 1, 2],
            description="Axis:",
        ...
        super().__init__(
            [
                self.plot,
                ipw.HBox([self.axis, self.dl_link]),
                self.height_slider,
    ...
    def update_plot(self, _=None):
        ...
        with self.plot:
            ...
            axplt.imshow(
                ...
                vmin=vmin,
                vmax=vmax,
                alpha=(1 - self.opacity_slider.value),
                zorder=10,
            )
    ...
    def make_export_link_txt(self, data):
        header = "xlim=({:.2f}, {:.2f}), ylim=({:.2f}, {:.2f})".format(
            0.0,
            self._current_structure.cell[0][0],
            0.0,
            self._current_structure.cell[1][1],
        )
        tempio = io.BytesIO()
        ...
        filename = f"{plot_name}_h{self.selected_height:.3f}.txt"
    ...
    def make_export_link_igor(self, data):
        ...
        filename = f"{plot_name}_h{self.selected_height:.3f}.itx"
        ...
        html = f'<a download="{filename}" href="'
        html += f'data:chemical/itx;name={filename};base64,{enc_file}"'
        html += ' id="export_link"'
...
class NanoribbonShowWidget(ipw.VBox):
    ...
    def __init__(self, workcalc, **kwargs):
        ...
        for orbitals_calc in self.orbitals_calcs:
            if "output_data_multiple" in orbitals_calc.outputs:
                ...
                self.list_of_calcs += [
                    (x.name, orbitals_calc)
                    for x in orbitals_calc.outputs.retrieved.list_objects()
                ]
        ...
        custom_cmap = matplotlib.colors.LinearSegmentedColormap.from_list(
            "half_seismic", custom_cmap_colors
        )

        self.orbital_viewer_2d = CubeArrayData2dViewerWidget(
            cmap=custom_cmap, center0=False, export_label=None
        )
        ...
        if self.spindensity_calc:
            self.spinden_viewer_2d = CubeArrayData2dViewerWidget(
                cmap="seismic", export_label="pk%d_spin" % workcalc.pk
            )
            self.spinden_viewer_3d = CubeArrayData3dViewerWidget()

            self.output_s = ipw.Output()

            self.spin_view_3D = ipw.RadioButtons(
                options=[("no", 0), ("yes", 1)],
        ...
        self.sts_mapping_viewer = CubeArrayData2dViewerWidget(
            cmap="gist_heat",
            center0=False,
            show_cbar=False,
            export_label="pk%d_ldos" % workcalc.pk,
        )
        ...
        super().__init__(
            [
                self.info,
                ipw.HBox([self.bands_viewer, self.output]),
                spin_density_vbox,
                self.sts_viewer_box,
            ],
            **kwargs,
    ...
    def _read_arraydata(self, i_spin, i_kpt, i_band):
        ...
        cube_id = [
            i
            for i, f in enumerate(self.list_of_calcs)
            if "K"
            + str(kpt_qe_convention).zfill(3)
            + "_"
            + "B"
            + str(i_band + 1).zfill(3)
            in list(f)[0]
        ]
    ...
    def on_kpoint_change(self, _=None):
        ...
        arraydata_fn = self._read_arraydata(
        ...
        if arraydata_fn is None:
            with self.output:
                ...
                display(
                    ipw.VBox(
                        [self.info_out], layout=ipw.Layout(margin="200px 0px 0px 0px")
                    )
                )
        else:
        ...
        for i_band in range(self.bands_lowres.shape[2] - 1, -1, -1):
            for i_spin in range(self.bands_lowres.shape[0]):
                for i_kpt in range(self.bands_lowres.shape[1]):
                    ...
                    orb_energy = (
                        self.bands_lowres[i_spin, i_kpt, i_band] - self.vacuum_level
                    )
    ...
    def on_sts_btn_click(self, _=None):
        sts_arraydata, warn = self.calculate_sts_mapping(
            self.sts_energy_text.value, self.sts_fwhm_text.value
        )
        ...
        if sts_arraydata is not None:
            ...
            self.sts_mapping_viewer_wrapper.children = [
                ipw.HTML(value="Could not find data.")
            ]
